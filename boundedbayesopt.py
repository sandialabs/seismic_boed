...
class BoundedBayesOpt:
    def __init__(
        ...
        # Only two filetypes suprported
            ...
            raise ValueError(
                f"bounds filename must have extension '.json' or '.kml', not '{bounds_ext}"
            )
    ...
    def run_opt(self, target_fun=None, n_iter=40, n_restarts=25):
        ...
        if self.target_fun is None:
            raise ValueError(
                "Must provide a target function either on initialization or in the args to this function"
            )
