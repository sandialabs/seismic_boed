...
def gen_power_normal(theta, sensors, ndata, stype):
    ...
    if mt_prior.mt_marginalization_enabled():
        ...
        for idata, mt_sample in enumerate(mt_samples):
            pred_log_power, sigma_sq_total, enabled_mask = lm.compute_power_distribution(
            ...
            power_data[idata, enabled_mask] = np.random.normal(
                pred_log_power[enabled_mask], sigma_total[enabled_mask]
            )
        return power_data
...
def generate_data(theta, sensors, ndata):
    ...
    if seismic_exists:
        seis_atimes, seis_detects, seis_azmths, seis_incdnts, seis_power = split_data(
            seismic_data
        )
    if instant_exists:
        inst_atimes, inst_detects, inst_azmths, inst_incdnts, inst_power = split_data(
            instant_data
        )
    if infra_exists:
        infra_atimes, infra_detects, infra_azmths, infra_incdnts, infra_power = (
            split_data(infra_data)
        )
    if array_exists:
        array_atimes, array_detects, array_azmths, array_incdnts, array_power = (
            split_data(array_data)
        )
